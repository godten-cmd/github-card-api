const projects = {
  ...
  function buildStackRow(stack, maxWidth) {
    ...
    for (const s of stack) {
      ...
      const badge = `<g transform="translate(${x}, 0)">
        ...
        <text x="${w / 2}" y="13.5" text-anchor="middle" font-family="'Segoe UI',sans-serif" font-size="11" fill="#5F5E5A">${s}</text>
      </g>`;
      badges.push(badge);
      x += w + 6;
    }
  ...
  module.exports = (req, res) => {
    ...
    <text x="${PAD}" y="36" font-family="'Segoe UI',sans-serif" font-size="15" font-weight="700" fill="${textPrimary}">${p.name}</text>
    ${tagSvg}
    ${lines.map((l, i) => `<text x="${PAD}" y="${62 + i * 19}" font-family="'Segoe UI',sans-serif" font-size="12" fill="${textSecondary}">${l}</text>`).join("\n  ")}
